...
#Function to save history data to a file
def save_history_to_file():
    with open(history_file, "w") as file:
        json.dump(history_data, file)

#Function to load history data from a file
...
def Hire():
    ...
    # Check if the same first name and last name already exist in history_data
    existing_entry = next((entry for entry in history_data if entry[0] == first_name_data and entry[1] == last_name_data), None)

    if existing_entry:
        # Ask if the user wants to reuse the same receipt number
        reuse_receipt = messagebox.askyesno("Reuse Receipt", f"An entry with the name {first_name_data} {last_name_data} already exists. Do you want to reuse the same receipt number {existing_entry[2]}?")
        
        if reuse_receipt:
            receipt = existing_entry[2]  # Reuse the existing receipt number
        else:
            receipt = random.randint(1, 100)  # Generate a new receipt number
    else:
        receipt = random.randint(1, 100)  # Generate a new receipt number if no matching entry is found
    ...
    # Log the entry to the file (optional, as we are already saving to JSON)
...
def Return_entry():
    ...
    global history_data
    history_data = [item for item in history_data if not all(
        str(val) == str(selected_values[i]) for i, val in enumerate(item)
    )]
    ...
    messagebox.showinfo("Return", "Item returned successfully!")

#History subroutine
def history():
    global history_tree

    #Reload the history data from the file
    ...
    history_window.configure(bg='#E2F3FA')
    ...
    history_title_label.config(font=("Courier", 20, "underline"), bg='#E2F3FA')
...
def setup_main_window():
    ...
    title_label.config(font=("Courier", 37, "underline"), bg="#E2F3FA")
...
canvas=Canvas(main_window, width=1000, height=800, bg="#E2F3FA", bd=0, highlightthickness=0)
